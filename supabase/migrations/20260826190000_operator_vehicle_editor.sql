-- Operator vehicle editor: one atomic, operator-scoped vehicle + captain + Route Offer contract.

alter table pace_v2.vehicle_route_offers
  add column if not exists below_minimum_operation_mode text;

update pace_v2.vehicle_route_offers
set below_minimum_operation_mode = case
  when min_value_threshold_ratio is null then 'route_default'
  else 'custom_threshold'
end
where below_minimum_operation_mode is null;

alter table pace_v2.vehicle_route_offers
  alter column below_minimum_operation_mode set default 'route_default',
  alter column below_minimum_operation_mode set not null,
  drop constraint if exists vehicle_route_offer_below_minimum_mode_check,
  add constraint vehicle_route_offer_below_minimum_mode_check check (
...
set below_minimum_operation_mode = coalesce(
  (select vro.below_minimum_operation_mode
   from pace_v2.vehicle_route_offers vro
   where vro.id = vc.vehicle_route_offer_id),
  case when vc.min_value_threshold_ratio is null then 'route_default' else 'custom_threshold' end
)
where vc.below_minimum_operation_mode is null;

alter table pace_v2.vehicle_considerations
  alter column below_minimum_operation_mode set default 'route_default',
  alter column below_minimum_operation_mode set not null,
  drop constraint if exists vehicle_consideration_below_minimum_mode_check,
  add constraint vehicle_consideration_below_minimum_mode_check check (
    below_minimum_operation_mode in ('never','route_default','custom_threshold')
    and (
      (below_minimum_operation_mode = 'custom_threshold' and min_value_threshold_ratio > 0 and min_value_threshold_ratio <= 1)
...
begin
  if new.vehicle_route_offer_id is not null then
    select vro.below_minimum_operation_mode,v.default_min_value_threshold_ratio
      into v_mode,v_default_ratio
    from pace_v2.vehicle_route_offers vro
    ...
    new.min_value_threshold_ratio := case
      when v_mode='never' then null
      when v_mode='route_default' then v_default_ratio
      else new.min_value_threshold_ratio
    end;
  end if;
  return new;
end;
...
drop trigger if exists trg_00_sync_consideration_below_minimum_mode
  on pace_v2.vehicle_considerations;
create trigger trg_00_sync_consideration_below_minimum_mode
before insert or update of vehicle_route_offer_id, min_value_threshold_ratio
on pace_v2.vehicle_considerations
for each row execute function pace_v2.sync_consideration_below_minimum_mode();
...
begin
  v_is_locked :=
    ...
    or exists (select 1 from pace_v2.booking_allocations ba where ba.vehicle_consideration_id = old.id)
    or exists (select 1 from pace_v2.confirmed_allocations ca where ca.consideration_id = old.id);
  ...
  if v_is_locked and (
    ...
    new.below_minimum_operation_mode is distinct from old.below_minimum_operation_mode or
  ...
  if v_is_locked then
    new.commercial_snapshot_locked_at := coalesce(old.commercial_snapshot_locked_at, now());
  end if;
  return new;
end;
$$;

create or replace view public.v2_operator_vehicle_editor
...
select
  ...
  v.capacity_seats,
  v.active,
  pref.captain_id as preferred_captain_id,
  concat_ws(' ', pc.first_name, pc.last_name) as preferred_captain_name
from pace_v2.vehicles v
join pace_v2.vehicle_types vt on vt.id = v.vehicle_type_id
left join lateral (
  select vcp.captain_id
  from pace_v2.vehicle_captain_preferences vcp
  where vcp.vehicle_id = v.id and vcp.operator_id = v.operator_id and vcp.active
  order by vcp.priority, vcp.created_at
  limit 1
) pref on true
left join pace_v2.captains pc on pc.id = pref.captain_id
where pace_v2.has_operator_access(v.operator_id);

create or replace view public.v2_operator_vehicle_editor_captains
with (security_invoker = true) as
select
  c.operator_id,
  c.id as captain_id,
  concat_ws(' ', c.first_name, c.last_name) as captain_name,
  c.email,
  cvt.vehicle_type_id
from pace_v2.captains c
join pace_v2.captain_vehicle_types cvt on cvt.captain_id = c.id and cvt.active
where c.active and pace_v2.has_operator_access(c.operator_id);

create or replace view public.v2_operator_vehicle_editor_types
with (security_invoker = true) as
select ovt.operator_id, vt.id as vehicle_type_id, vt.name as vehicle_type_name
from pace_v2.operator_vehicle_types ovt
join pace_v2.vehicle_types vt on vt.id=ovt.vehicle_type_id and vt.active
where ovt.status='approved' and pace_v2.has_operator_access(ovt.operator_id);

create or replace view public.v2_operator_vehicle_editor_routes
with (security_invoker = true) as
select distinct
  o.id as operator_id,
  r.id as route_id,
  ...
  r.country_id,
  r.locality_id
from pace_v2.operators o
join pace_v2.countries c on c.id = o.country_id and c.active
join pace_v2.routes r
  on r.country_id = o.country_id
 and r.is_active
 and (not c.is_large or r.locality_id = o.locality_id)
join pace_v2.route_vehicle_types rvt
  on rvt.route_id = r.id
 and rvt.active
 and rvt.effective_from <= now()
 and (rvt.effective_to is null or rvt.effective_to > now())
join pace_v2.operator_vehicle_types ovt
  on ovt.operator_id = o.id
 and ovt.vehicle_type_id = rvt.vehicle_type_id
 and ovt.status = 'approved'
where o.active and pace_v2.has_operator_access(o.id);

create or replace view public.v2_operator_vehicle_editor_offers
with (security_invoker = true) as
select
  vro.id as offer_id,
  v.operator_id,
  ...
  vro.below_minimum_operation_mode,
  vro.post_min_discount_enabled,
  vro.post_min_discount_bps,
  vro.effective_from,
  vro.effective_to
from pace_v2.vehicle_route_offers vro
join pace_v2.vehicles v on v.id = vro.vehicle_id
...
grant select on public.v2_operator_vehicle_editor,
  public.v2_operator_vehicle_editor_captains,
  public.v2_operator_vehicle_editor_types,
  public.v2_operator_vehicle_editor_routes,
  public.v2_operator_vehicle_editor_offers to authenticated;
revoke all on public.v2_operator_vehicle_editor,
  public.v2_operator_vehicle_editor_captains,
  public.v2_operator_vehicle_editor_types,
  public.v2_operator_vehicle_editor_routes,
  public.v2_operator_vehicle_editor_offers from anon;
