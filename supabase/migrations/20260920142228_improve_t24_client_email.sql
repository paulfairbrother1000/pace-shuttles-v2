-- Existing pickup records predate mandatory operational meeting instructions.
-- Give them a safe editable baseline so current customers continue to receive
-- T-24 reminders; Site Admin can replace this with precise pier or desk detail.
update pace_v2.pickup_points
set arrival_notes='Please follow the Google Maps directions below to '||name||'. On arrival, go to the main passenger meeting area and look for your assigned Pace Shuttles vehicle.',
    updated_at=now()
where active
  and nullif(trim(coalesce(arrival_notes,'')),'') is null;
...
begin
  for v_row in
    select
      ...
      pp.arrival_notes as pickup_arrival_notes,
  ...
  loop
    ...
    if nullif(trim(coalesce(v_row.pickup_arrival_notes,'')),'') is null then v_missing:=array_append(v_missing,'missing pickup instructions'); end if;
    ...
    if v_row.journey_pair_id is not null then
      ...
      v_body:=
        ...
        v_row.vehicle_type||': '||v_row.vehicle_name||E'\n\nCaptain: '||v_row.captain_first_name||' '||v_row.captain_last_name||
        E'\n\nItinerary\n\n'||
        concat_ws(
          ' and ',
          case when v_row.adult_count>0 then v_row.adult_count||' '||case when v_row.adult_count=1 then 'adult' else 'adults' end end,
          case when v_row.child_count>0 then v_row.child_count||' '||case when v_row.child_count=1 then 'child' else 'children' end end,
          case when v_row.infant_count>0 then v_row.infant_count||' '||case when v_row.infant_count=1 then 'infant' else 'infants' end end
        )||
        ...
        to_char((v_row.scheduled_departure_ts-interval '15 minutes') at time zone coalesce(nullif(v_row.outbound_timezone,''),v_row.timezone),'FMHH12:MI AM')||
        ...
        E'\n\nArrival at '||v_row.pickup_name||E'\n\n'||v_row.pickup_arrival_notes||
        E'\n\nPlease arrive at '||v_row.pickup_name||' by '||
        to_char((v_row.scheduled_departure_ts-interval '15 minutes') at time zone coalesce(nullif(v_row.outbound_timezone,''),v_row.timezone),'FMHH12:MI AM')||
        E'.\n\nDirections are available at the following link:\n'||v_row.pickup_directions_url||
        E'\n\nIf you have trouble locating the '||v_row.vehicle_type||', please contact Captain '||v_row.captain_last_name||E' directly using the instructions below.'||
      ...
      v_metadata:=jsonb_build_object(
        ...
        'pickup_arrival_notes',v_row.pickup_arrival_notes,
