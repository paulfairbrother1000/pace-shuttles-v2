-- Qualify candidate aliases for PostgreSQL PL/pgSQL output-column name resolution.
...
begin
  ...
  with ordered as (
    ...
    select distinct on(rc.consideration_id) rc.* from raw_candidates rc order by rc.consideration_id,rc.offer_price,rc.use_discount desc
  ), ranked as (
    select d.*,row_number() over(order by d.offer_price,d.quality_score desc,d.seq,d.consideration_id)::integer offer_rank from deduped d
  )
