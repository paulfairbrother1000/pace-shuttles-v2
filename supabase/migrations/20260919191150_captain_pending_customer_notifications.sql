create unique index if not exists customer_notifications_one_captain_pending_per_booking_departure
  on pace_v2.notifications(booking_id,departure_id)
  where template_code='journey_captain_pending';

create or replace function pace_v2.queue_captain_pending_t24_notifications(p_as_of timestamptz)
returns integer
language plpgsql
...
begin
  for v_row in
    ...
    join lateral (
      select ad.decision_reason_code
      from pace_v2.allocation_decisions ad
      where ad.departure_id=d.id
      order by ad.created_at desc,ad.id desc
      limit 1
    ) latest_decision on latest_decision.decision_reason_code='T24_INSUFFICIENT_CAPTAINS'
  ...
  loop
    insert into pace_v2.notifications(
      ...
      jsonb_build_object(
        ...
        'departure_date_label',to_char(v_row.scheduled_departure_ts at time zone v_row.timezone,'FMDay, FMDD FMMonth YYYY'),
        'departure_time_label',to_char(v_row.scheduled_departure_ts at time zone v_row.timezone,'FMHH12:MI AM'),
        'arrival_by_time_label',to_char((v_row.scheduled_departure_ts-interval '15 minutes') at time zone v_row.timezone,'FMHH12:MI AM'),
        'vehicle_type',v_row.vehicle_type,
        'vehicle_name',v_row.vehicle_name,
        'pickup_directions_url',v_row.pickup_directions_url,
        'wet_destination',v_row.wet_or_dry='wet',
        'captain_status','to_be_confirmed'
      )
    )
    on conflict (booking_id,departure_id) where template_code='journey_captain_pending' do nothing;
...
revoke all on function pace_v2.queue_captain_pending_t24_notifications(timestamptz) from public,anon,authenticated;
