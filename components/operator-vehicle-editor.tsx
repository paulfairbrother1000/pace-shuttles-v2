 ...
 return <section className="vehicle-workspace" aria-label="Fleet and vehicle editor">
  ...
  <div className="vehicle-editor">
   ...
   <div className="editor-panel"><h3>Vehicle details</h3><div className="vehicle-form-grid">
    ...
    <label><span>Default captain</span><select aria-label="Default / preferred captain" value={draft.preferredCaptainId} onChange={e=>update('preferredCaptainId',e.target.value)} disabled={!draft.vehicleTypeId}><option value="">Select captain</option>{captainOptions.map(c=><option key={c.captain_id} value={c.captain_id}>{c.captain_name}</option>)}</select>{fieldError(errors,'preferredCaptainId')}<small>Required while this vehicle is active. The captain may be the operator owner.</small></label>
