...
export async function dispatchDueCustomerEmails(limit=25,deps:CustomerEmailDependencies={}){
 ...
 for(const row of rows){
  try{
   ...
   const subject=t72?.subject||tomorrow?.subject||captainPending?.subject||feedback?.subject||broadcast?.subject||row.subject||'Pace Shuttles update';const text=t72?.text||tomorrow?.text||captainPending?.text||feedback?.text||broadcast?.text||row.body||'';
