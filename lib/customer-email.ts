...
import {buildT24OperatorEmail,type T24OperatorEmailInput} from './t24-operator-email';
import {buildCaptainPendingJourneyEmail,buildTomorrowJourneyEmail,type CaptainPendingJourneyEmailInput,type TomorrowJourneyEmailInput} from './journey-email-content';
...
type TomorrowMetadata={first_name:string;country_name:string;pickup_name:string;destination_name:string;outbound_pickup_time_label:string;outbound_arrival_by_time_label:string;return_pickup_time_label:string;return_arrival_by_time_label:string;adult_count:number;child_count:number;infant_count:number;captain_full_name:string;captain_surname:string;vehicle_type:string;vehicle_name:string;pickup_arrival_notes:string;pickup_directions_url:string;wet_destination:boolean};
...
function hasTomorrowMetadata(value:unknown):value is TomorrowMetadata{
 if(!value||typeof value!=='object')return false;
 const metadata=value as Record<string,unknown>;
 const strings=['first_name','country_name','pickup_name','destination_name','outbound_pickup_time_label','outbound_arrival_by_time_label','return_pickup_time_label','return_arrival_by_time_label','captain_full_name','captain_surname','vehicle_type','vehicle_name','pickup_arrival_notes','pickup_directions_url'];
 const counts=['adult_count','child_count','infant_count'];
 return strings.every(key=>typeof metadata[key]==='string'&&Boolean(String(metadata[key]).trim()))
  &&counts.every(key=>Number.isInteger(metadata[key])&&Number(metadata[key])>=0)
  &&typeof metadata.wet_destination==='boolean';
}
...
export async function dispatchDueCustomerEmails(limit=25,deps:CustomerEmailDependencies={}){
 ...
 for(const row of rows){
  try{
   ...
   const t24Operator=row.template_code==='T24_OPERATOR_CONFIRMED_EMAIL'&&row.metadata?buildT24OperatorEmail(row.metadata as unknown as T24OperatorEmailInput):null;
   ...
   const tomorrowMetadata=hasTomorrowMetadata(row.metadata)?row.metadata:null;
   const tomorrow=row.template_code==='journey_tomorrow'&&tomorrowMetadata?buildTomorrowJourneyEmail({firstName:tomorrowMetadata.first_name,countryName:tomorrowMetadata.country_name,pickupName:tomorrowMetadata.pickup_name,destinationName:tomorrowMetadata.destination_name,outboundPickupTimeLabel:tomorrowMetadata.outbound_pickup_time_label,outboundArrivalByTimeLabel:tomorrowMetadata.outbound_arrival_by_time_label,returnPickupTimeLabel:tomorrowMetadata.return_pickup_time_label,returnArrivalByTimeLabel:tomorrowMetadata.return_arrival_by_time_label,adultCount:tomorrowMetadata.adult_count,childCount:tomorrowMetadata.child_count,infantCount:tomorrowMetadata.infant_count,captainFullName:tomorrowMetadata.captain_full_name,captainSurname:tomorrowMetadata.captain_surname,vehicleType:tomorrowMetadata.vehicle_type,vehicleName:tomorrowMetadata.vehicle_name,pickupArrivalNotes:tomorrowMetadata.pickup_arrival_notes,pickupDirectionsUrl:tomorrowMetadata.pickup_directions_url,wetDestination:tomorrowMetadata.wet_destination} satisfies TomorrowJourneyEmailInput):null;
   const subject=t24Operator?.subject||t72?.subject||tomorrow?.subject||captainPending?.subject||feedback?.subject||broadcast?.subject||row.subject||'Pace Shuttles update';const text=t24Operator?.text||t72?.text||tomorrow?.text||captainPending?.text||feedback?.text||broadcast?.text||row.body||'';
   const response=await fetchImpl('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${resend}`,'Content-Type':'application/json','Idempotency-Key':`pace-notification-${row.notification_id}`},body:JSON.stringify({from:env.RESEND_FROM_EMAIL||'Pace Shuttles <hello@paceshuttles.com>',to:[row.to_email],subject,text,html:t24Operator?.html||tomorrow?.html||renderCustomerEmailHtml(subject,text,row.template_code)})});
